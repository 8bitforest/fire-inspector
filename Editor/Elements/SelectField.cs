...
{
    public class SelectField<T> : BaseField<T>
    {
        ...
        public SelectField(string label, List<SelectOption<T>> options) : base(label, new VisualElement())
        {
            _options = options;
            ...
            RegisterCallback<ChangeEvent<SelectOption<T>>>(_ => { UpdateLabel(); });
        }
        ...
        private void TogglePopup()
        {
            try
            {
                ...
                if (closed)
                {
                    ...
                    var selected = _options.Where(o => o.Value.Equals(value)).Cast<SelectOption>().ToList();
                    var options = new List<SelectOption>(_options);

                    _popup.Initialize(options, selected, selections =>
                    {
                        value = (T)selections[0].Value;
                        _popup.Close();
                        _popup = null;
                    });
        ...
        {
            var option = _options.Find(o => o.Value.Equals(value));
            if (option?.Icon != null)
            {
                _iconElement.style.display = DisplayStyle.Flex;
                _iconElement.image = option.Icon;
            }
            else
            {
            ...
            _textElement.text = option?.Text ?? string.Empty;
        }

        private class SelectTextElement : TextElement { }
    }

    public class SelectPopupWindow : EditorWindow
    {
